...
public class Main {
		...
		while (!input.equals("0")) {
			...
			if (idx > 0 && idx <= inventories.get(toSell).getItems().size()) {
				...
				if (amount > 0 && amount <= inventories.get(toSell).getItems().get(idx - 1).getCount()) {
					ItemUtil.playerSellplayer(inventories.get(toSell), inventories.get(toBuy), inventories.get(toSell).getItems().get(idx-1).getItem(), amount);
				} else {
					System.out.println("Invalid Amount!");
				}
			} else if (idx != 0) {
	...
	public static void showSellItemToMarket(int toSell) {
		...
		while (!input.equals("0")) {
			...
			if (idx > 0 && idx <= inventories.get(toSell).getItems().size()) {
				...
				if (amount > 0 && amount <= inventories.get(toSell).getItems().get(idx - 1).getCount()) {
					ItemUtil.playerSellMarket(inventories.get(toSell), inventories.get(toSell).getItems().get(idx-1).getItem(), amount);
				} else {
					System.out.println("Invalid Amount!");
				}
			} else if (idx != 0) {
	...
	public static void showAddItemFromFile() {
		...
		ArrayList<Item> itemToAdd = ItemUtil.getItemFromFile(fileName);
